...
def load_base_rate():
    base_rate = pd.read_csv("datasets/FEDFUNDS.csv")
    return base_rate.set_index('DATE')


def load_treasury_yeild():
    treasury_10_rate = pd.read_csv("datasets/DGS10.csv")
    return treasury_10_rate.set_index('DATE')
...
def build_dataset():
    ...
    msft_data['FEDFUNDS'] = msft_data['FEDFUNDS'].fillna(method='pad')
